...
public class kamar_suite extends AppCompatActivity {
    public static int harganya = 600000;
        ...
        next.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if(nama.getText().toString().trim().length() <=0 || hp.getText().toString().trim().length() <=0 || alamat.getText().toString().trim().length() <=0 ){
                    Toast.makeText(kamar_suite.this,"Data Harus Dilengkapi", Toast.LENGTH_LONG).show();
                }else{
                    ...
                    Intent i =new Intent (getApplicationContext(),lanjutan_suite.class);
                    startActivity(i);
                }
